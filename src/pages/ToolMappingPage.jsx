...
import { getTools, getControls, getToolMappings, addToolMapping, removeToolMapping } from '../api';
...
export const ToolMappingPage = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      const [tRes, cRes, mRes] = await Promise.all([
        getTools(), getControls(), getToolMappings()
      ]);
      if (tRes.data?.success) setTools(tRes.data.data);
      if (cRes.data?.success) setControls(cRes.data.data);
  ...
  const handleToggleMapping = async (controlId) => {
    ...
    const existing = mappings.find(m => 
      ...
      (m.controlId._id === controlId || m.controlId === controlId)
    );

    try {
      if (existing) {
        await removeToolMapping(existing._id);
        setMappings(prev => prev.filter(m => m._id !== existing._id));
      } else {
        const res = await addToolMapping({ toolId: selectedTool._id, controlId, verified: true });
        if (res.data?.success) {
          // Re-fetch to get populated mappings for accurate display, or construct manually
          // Simple manual construction:
          const newMapping = {
            _id: res.data.data.mapping._id,
            toolId: selectedTool,
            controlId: controls.find(c => c._id === controlId),
            verified: true
          };
  ...
  const filteredControls = controls.filter(c => 
    (c.title || c.controlName || '').toLowerCase().includes(controlSearch.toLowerCase()) ||
  ...
  return (
    <div className="space-y-6 h-[calc(100vh-8rem)] flex flex-col">
      <div className="shrink-0">
        <Breadcrumb items={[{ label: 'Tool Mapping' }]} />
        <div className="mt-2">
          <h1 className="text-2xl font-bold text-[#0D1514]">Control-Tool Mapping</h1>
          <p className="text-[#64748B] text-sm mt-1">Map security tools to the controls they address to calculate coverage.</p>
        </div>
      </div>
          ...
          <div className="md:col-span-7 bg-white/60 backdrop-blur-xl border border-white/40 rounded-xl shadow-glass flex flex-col h-full overflow-hidden">
            <div className="p-4 border-b border-[#F1F5F9] shrink-0 bg-[#F8FAFC]/50">
              <h2 className="font-semibold text-[#0D1514] flex items-center gap-2 mb-3">
                ...
                {selectedTool ? `Controls covered by ${selectedTool.name || selectedTool.toolName}` : 'Select a tool to map controls'}
              </h2>
              <div className="relative">
                <Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-[#94A3B8]" />
                <input
                  type="text" placeholder="Search controls..."
            ...
            <div className="flex-1 overflow-y-auto p-4">
              {!selectedTool ? (
                <div className="h-full flex flex-col items-center justify-center text-[#94A3B8] space-y-3">
                  ...
                  <p className="text-sm">Select a tool from the left panel to manage its control mappings.</p>
                </div>
              ) : (
                <div className="space-y-2">
                  {filteredControls.map(c => {
                    const isMapped = mappings.some(m => 
                      (m.toolId._id === selectedTool._id || m.toolId === selectedTool._id) && 
                      (m.controlId._id === c._id || m.controlId === c._id)
                    );
                      ...
                      <div key={c._id} className={`flex items-center justify-between p-3 rounded-xl border transition-all ${
                        ...
                        <div className="flex-1 min-w-0 pr-4">
                          <div className="flex items-center gap-2">
                            <span className="text-xs font-mono font-semibold text-[#64748B]">{c.controlId}</span>
                            <span className="text-sm font-medium text-[#0D1514] truncate">{c.title || c.controlName}</span>
                          </div>
                          <div className="flex items-center gap-2 mt-1">
                            <span className={`text-[10px] px-1.5 py-0.5 rounded-full ${STATUS_COLORS[c.lifecycleStage || 'Defined'] || STATUS_COLORS['Defined']}`}>
                              {c.lifecycleStage || 'Defined'}
                            </span>
                            {c.atRisk && <span className="text-[10px] text-red-500 font-semibold px-1.5 py-0.5 rounded-full bg-red-50 border border-red-200">At Risk</span>}
                          </div>
                        </div>
                        ...
                        {isAdmin && (
                          ...
                          >
                            {working ? <Loader2 size={14} className="animate-spin" /> :
                             isMapped ? <><Unlink size={14}/> Remove</> : <><LinkIcon size={14}/> Map Control</>
                            }
                          </button>
                        )}
                        {!isAdmin && isMapped && (
